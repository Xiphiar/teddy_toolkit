...
main = async() => {
    for (let i = 0; i < ids.length; i++) {
        if (!fileExists(`data/encrypted/${ids[i]}`) || !fileExists(`data/keys/out-${ids[i]}.json`)) {

            //get original image file
            const img = fs.readFileSync(`data/ALL/${ids[i]}.png`);

            //prepare POST data
            const formData = new FormData();
            formData.append("data", img, `${ids[i]}.png`);
            ...
            try {
                ...
                const res = await retry(
                    () => {
                        console.log(`trying to upload data/ALL/${ids[i]}.png`)
                        return axios.post(
                            `https://stashhapp-public-testnet.azurewebsites.net/upload`,
                            formData,
                            {
                                headers: {
                ...
                fs.writeFile(`data/keys/error-${ids[i]}.json`, output, (err) => {
                    if (err) {
                        throw err;
                    }
                });
    ...
    fs.writeFile('data/out.json', data2, (err) => {
        if (err) {
            throw err;
        }
        console.log("Master JSON data is saved.");
    });
}
main();
